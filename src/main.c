...
int main(int argc, char *argv[]) {
    ...
    pthread_t thread_id;
    thread_par par;
    par.conn = PQconnectdb("postgresql://user:admin@local_postgresdb:5432/postgres"); //sudo docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' container_name
